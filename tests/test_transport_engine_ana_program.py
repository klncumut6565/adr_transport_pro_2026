"""Ana uygulama motoru (main.py ADREngine) — mevzuat hesap testleri.
...
class TestPointsUseRealCategory:
    def test_category1_x50(self):
        ...
        total, required, _ = M.ADREngine.calculate_1136_points([item(qty=30, tc="1")])
        assert total == 1500
        assert required is True

    def test_category3_x1(self):
        total, required, _ = M.ADREngine.calculate_1136_points([item(qty=800, tc="3")])
        assert total == 800
        assert required is False

    def test_category2_x3(self):
        total, _, _ = M.ADREngine.calculate_1136_points([item(qty=100, tc="2")])
        assert total == 300

    def test_category4_zero_points(self):
        total, required, _ = M.ADREngine.calculate_1136_points([item(qty=5000, tc="4")])
        assert total == 0
        assert required is False

    def test_category0_no_exemption_possible(self):
        # Kategori 0: miktar ne olursa olsun muafiyet YOK, plaka zorunlu
        total, required, _ = M.ADREngine.calculate_1136_points([item(qty=0.1, tc="0")])
        assert required is True

    def test_mixed_categories_sum(self):
        items = [item(qty=10, tc="1"), item(un="1830", cls="8", qty=100, tc="2"),
                 item(un="3082", cls="9", qty=100, tc="3")]
        total, required, _ = M.ADREngine.calculate_1136_points(items)
    ...
    def test_unknown_category_conservative(self):
        ...
        total, required, msg = M.ADREngine.calculate_1136_points([item(qty=100, tc="")])
        assert total >= 100 * 50
        assert required is True
        assert "kategori" in msg.lower()

    def test_boundary_exactly_1000(self):
        total, required, _ = M.ADREngine.calculate_1136_points([item(qty=1000, tc="3")])
        assert total == 1000 and required is False
        total, required, _ = M.ADREngine.calculate_1136_points([item(qty=1001, tc="3")])
        assert required is True
...
class TestExemptionType:
    def test_500_points_is_1136_exempt(self):
        report = M.ADREngine.generate_adr_report([item(qty=500, tc="3")])
        assert report.exemption_type == M.ExemptionType.ADR_1_1_3_6.value
        assert report.orange_plate_required is False

    def test_over_limit_no_exemption(self):
        report = M.ADREngine.generate_adr_report([item(qty=2000, tc="3")])
        assert report.exemption_type == M.ExemptionType.NONE.value
        assert report.orange_plate_required is True

    def test_all_lq_is_lq_exemption(self):
        report = M.ADREngine.generate_adr_report([item(lq=True), item(un="1830", lq=True)])
        assert report.exemption_type == M.ExemptionType.LQ.value
...
class TestTunnelCodes:
    def test_combined_code_most_restrictive(self):
        assert M.ADREngine.calculate_tunnel_restriction([item(tunnel="D/E")]) == "D"

    def test_parenthesized(self):
        assert M.ADREngine.calculate_tunnel_restriction([item(tunnel="(C/D)")]) == "C"

    def test_mixed_items_strictest_wins(self):
        items = [item(tunnel="E"), item(un="1830", tunnel="B/D"), item(un="3082", tunnel="D/E")]
        assert M.ADREngine.calculate_tunnel_restriction(items) == "B"

    def test_plain_codes_still_work(self):
        assert M.ADREngine.calculate_tunnel_restriction([item(tunnel="C")]) == "C"
        assert M.ADREngine.calculate_tunnel_restriction([item(tunnel="")]) == "E"
        assert M.ADREngine.calculate_tunnel_restriction([]) == "E"
...
class TestDriverCertificate:
    def _driver(self, expiry):
        return M.Driver(full_name="Test", adr_certificate_no="TR-1",
...
class TestItemPersistence:
    def test_roundtrip_preserves_safety_fields(self, tmp_path):
        db = M.DatabaseManager(str(tmp_path / "test.db"))
        sh = M.Shipment(document_no="T-1", document_date="2026-07-05")
    ...
    def test_existing_db_migrates(self, tmp_path):
        ...
        db = M.DatabaseManager(str(p))  # migrasyon burada çalışmalı
