"""webcore.ADREngine — Sınırlı Miktar (LQ, ADR 3.4) ve İstisnai Miktar
(EQ, ADR 3.5) testleri (Faz 4).

Masaüstü tests/test_lq_eq_ana_program.py'nin webcore karşılığı.
...
from webcore import ADREngine, Chemical, DatabaseManager, ShipmentItem

ROOT = Path(__file__).resolve().parent.parent


def chem(lq="", eq="", cls="3", **kw):
    return Chemical(un_number="1203", proper_shipping_name_tr="TEST",
...
class TestEqCodes:
    ...
    def test_lowercase_and_spaces_tolerated(self):
        assert ADREngine.eq_limits(" e2 ") == (30, 500)
...
class TestLqEligibility:
    def test_within_limit(self):
        ok, msg, mx = ADREngine.check_lq_eligibility(chem(lq="1 L"), 0.9, "L")
        assert ok and mx == 1.0 and "1" in msg

    def test_over_limit(self):
        ok, msg, mx = ADREngine.check_lq_eligibility(chem(lq="1 L"), 1.5, "L")
        assert not ok and mx == 1.0

    def test_zero_means_forbidden(self):
        ok, _, mx = ADREngine.check_lq_eligibility(chem(lq="0"), 0.1, "L")
        assert not ok and mx == 0.0

    def test_boundary_exact(self):
        ok, _, _ = ADREngine.check_lq_eligibility(chem(lq="5 kg"), 5.0, "kg")
        assert ok

    def test_unit_mismatch_flagged(self):
        # Limit litre, giriş kg: sessizce karşılaştırılamaz — uyarı içermeli
        ok, msg, _ = ADREngine.check_lq_eligibility(chem(lq="1 L"), 0.5, "kg")
    ...
    def test_per_substance_not_per_class(self):
        ...
        ok1, _, mx1 = ADREngine.check_lq_eligibility(chem(lq="1 L", cls="3"), 2.0, "L")
        ok2, _, mx2 = ADREngine.check_lq_eligibility(chem(lq="5 L", cls="3"), 2.0, "L")
...
class TestEqEligibility:
    def test_e2_within(self):
        ok, msg, mx = ADREngine.check_eq_eligibility(chem(eq="E2"), 25)
        assert ok and mx == 30 and "E2" in msg

    def test_e2_over(self):
        ok, _, _ = ADREngine.check_eq_eligibility(chem(eq="E2"), 31)
        assert not ok

    def test_e0_forbidden(self):
        ok, msg, mx = ADREngine.check_eq_eligibility(chem(eq="E0"), 0.5)
        assert not ok and mx == 0 and "E0" in msg

    def test_e5_tight_limit(self):
        ok, _, _ = ADREngine.check_eq_eligibility(chem(eq="E5"), 1)
        assert ok
        ok, _, _ = ADREngine.check_eq_eligibility(chem(eq="E5"), 2)
        assert not ok
...
class TestDerivedFlagsAndPersistence:
    def test_flags_derived_from_limits(self):
        c = chem(lq="1 L", eq="E2")
        assert ADREngine.is_lq_allowed(c) is True
        assert ADREngine.is_eq_allowed(c) is True
        c2 = chem(lq="0", eq="E0")
        assert ADREngine.is_lq_allowed(c2) is False
        assert ADREngine.is_eq_allowed(c2) is False

    def test_legacy_boolean_respected_when_no_limit_text(self):
        # Eski kayıtlar: limit metni yok ama boolean var — kaybolmamalı
        c = Chemical(un_number="1789", class_code="8", lq_allowed=True)
        assert ADREngine.is_lq_allowed(c) is True
...
# =========================================================================
# ADR Tablo A Excel -> chemicals aktarımı limitleri taşımalı
# (üç veri kaynağının birleşme noktası)
# =========================================================================
class TestRealExcelChemicalImport:
    """Gerçek ADR Tablo A Excel'i içe aktarılır (JSON değil)."""
...
class TestReportEnforcement:
    def _item(self, lq=False, eq=False, qty=10.0, count=1, lq_max=0.0, eq_max=0.0):
        return ShipmentItem(un_number="1203", proper_name="TEST", class_code="3",
